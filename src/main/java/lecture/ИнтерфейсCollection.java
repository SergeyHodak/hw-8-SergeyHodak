...
здесь - https://habr.com/ru/post/128269/.
    ...
    LinkedList#
    LinkedList — ещё одна реализация List. Основное отличие - внутрення реализация основана на двунаправленном связном списке, что теоретически позволяет быстрее вставлять-удалять элементы из середины очереди.

Эту коллекцию, из-за ее особенностей внутренней реализации, можно использовать как стек или очередь.

На Хабре есть статья с подробным анализом и описанием этой коллекции.

Stack и Vector#
Эти коллекции уже не используются на практике, но еще могут встретиться в старом коде.

Vector — реализация динамического массива объектов. Появился в Java 1.0, сейчас не рекомендуется использовать его. Как альтернативу используйте ArrayList.

Stack — данная коллекция является расширением коллекции Vector. Была добавлена в Java 1.0 как реализация стека LIFO (last-in-first-out). Сейчас не используется, рекомендуется использовать вместо этого реализации интерфейса Deque (например, ArrayDeque).

Работа с List#
Обычно List используется как удобная альтернатива массиву. Не нужно следить за размером, просто удобно работаем, а нужная память выделяется автоматически.

Наиболее частая операция - проход по всем элементам. Обычно реализуется циклом foreach, но можно реализовать и циклом со счетчиком:
...
Внутри использует объект HashMap для хранения данных. В качестве ключа используется добавляемый элемент, а в качестве значения — объект-пустышка (new Object()). Из-за особенностей реализации порядок элементов не гарантируется при добавлении.

Пример использования HashSet (для других реализаций Set подход аналогичный):
...
В примере выше в множестве uniqueNames останутся лишь строки John, Elon и Bill, причем их порядок не определен (зависит от реализации hashCode() у класса String, может отличаться от одной виртуальной машины к другой).

Пройтись по всем элементам мы можем лишь с помощью цикла foreach. Метода get(), который возвращал бы элемент по индексу, здесь нет.

Часто нужно узнать, есть ли элемент во множестве. Для этого есть метод contains():
...
Помимо методов, определённых в интерфейсе Collection, интерфейс Queue определяет дополнительные методы для извлечения и добавления элементов в очередь.
...
ПОЛЕЗНО
Queue работает как очередь в магазине. Элементы обслуживаются в порядке их поступления. Кто первый вошел, того первым и обслужат.

ArrayDeque#
ArrayDeque — реализация интерфейса Deque, который расширяет интерфейс Queue методами, позволяющими реализовать конструкцию вида LIFO (last-in-first-out).

Интерфейс Deque и реализация ArrayDeque были добавлены в Java 1.6. Эта коллекция представляет собой реализацию с использованием массивов, подобно ArrayList, но не позволяет обращаться к элементам по индексу и хранение null.
...
John
Bill
В примере выше мы добавляем элементы в очередь, используя метод add(). Дальше с помощью метода poll() мы получаем последний элемент, одновременно удаляя его.

PriorityQueue#
PriorityQueue является единственной прямой реализацией интерфейса Queue.

Особенность PriorityQueue - сортировка элементов при их добавлении. По умолчанию используется natural ordering. Но мы можем также передать свой Comparator, и изменить порядок сортировки.

Работа с Queue#
Типичное использование Queue - организация очереди задач. Например, у нас есть сайт с картинками. Когда пользователь добавляет картинку на сайт, ее вначале нужно обработать (обрезать, переконвертировать в нужный формат и так далее).

Это трудоемкий процесс, и при добавлении картинок мы помещаем каждую в очередь. Дальше фоновый поток берет каждую картинку, обрабатывает, и добавляет на сайт. Примерно так работает Youtube, когда вы добавляете видео.
...
Использование очереди в данном случае удобно тем, что мы обрабатываем фотографии в порядке их поступления. Чем раньше добавлена фотография, тем раньше она обработается и добавится на сайт, что вполне логично.
 */